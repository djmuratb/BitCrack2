...
cudaError_t CudaDeviceKeys::reInitializeBasePoints()
{
	// generate a table of points G, 2G, 4G, 8G...(2^255)G
	std::vector<secp256k1::ecpoint> table;
...
cudaError_t CudaDeviceKeys::reInit(int blocks, int threads, int pointsPerThread, const std::vector<secp256k1::uint256>& privateKeys)
{
	_blocks = blocks;
	_threads = threads;
	_pointsPerThread = pointsPerThread;
	...
	// Clear private keys
	cudaError_t err = cudaMemset(_devPrivate, 0, sizeof(unsigned int) * count * 8);
	if (err) {
		return err;
	}
